...
在 Ubuntu 上部署 **Gitea** 是设置本地代码托管平台的简洁和高效选择。
以二进制安装/配置. 以下是详细步骤：
...
```bash
# gitea --version
gitea version 1.26.4 built with go1.26.3-X:jsonv2 : bindata, sqlite, sqlite_unlock_notify
```
...
需要替换最新版本，可以在 [Gitea](https://github.com/go-gitea/gitea/releases) 的官方下载页面找到最新版本的下载链接
...
sudo touch -p /etc/gitea/app.ini
sudo chown -R root:git /etc/gitea
sudo chmod 770 /etc/gitea
```

### **步骤 7：配置app.ini**
sudo vi /etc/gitea/app.ini <br>
因为没启动安装,所以以下文本是新增的,第一次启动配置后,内容会追加.
```
[server]
# 新增一行，启用内置 SSH 服务器
START_SSH_SERVER = true
# 显示在网页中的 SSH 端口,因为路由器配置了端口转发 18084:8084
SSH_PORT = 18084
# 内置 SSH 服务器的监听端口
SSH_LISTEN_PORT = 8084
SSH_DOMAIN = git.seantang.top
DOMAIN = git.seantang.top
HTTP_PORT = 3000
```
...
tcp        0      0 127.0.0.1:47336         127.0.0.1:3000          ESTABLISHED
tcp        0      0 127.0.0.1:56306         127.0.0.1:3000          TIME_WAIT
```

### **步骤 11：访问 Gitea**

打开浏览器，访问 `http://<your-server-ip>:3000`。
按照界面完成安装配置，使用默认或手动配置数据库。使用sqlite已经足够了.

完成后即可使用 Gitea 提供的 Web 界面进行代码托管和管理。

- 首次登陆时需要设置文件目录等。<br>
- 注意设置管理员账号密码.
- 如果是企业内部使用,建议关闭openid注册.
- 3000端口是默认http端口,处于安全考虑,一般都需要配置https,建议采用 caddy进行端口转发,自动配置tls证书.
...
`--config /etc/gitea/app.ini`手动指定配置文件的路径
